...
class Population:
    ...
    def __init__(self, size, chrom_size, cp, mp, gen_max):
        ...
        for i in range(self.size):
            ##因为目标函数是二维的，所以这里也是二维的
            ##一个个体包含一组染色体共两条，都是用25位的二进制表示的，转为十进制数后范围是（0-v）
            ##解码时只需用到编码后的十进制表示的数
    ...
    def fitness_func(self, chrom1, chrom2):
        ...
        就是将染色体通过0-1编码，解码后为目标函数的定义域，代入目标函数进行计算即可得到适应度函数
        '''
        interval = [-10.0, 10.0]
    ...
    def select(self):
        ...
        (t, i) = (random.random(), 0)
        #相对而言，个体适应度越高，那么累计加上这个个体，超出已知随机数的可能性越大，那么该个体越可能被保留，当然，个体适应度低的也可能被保留
